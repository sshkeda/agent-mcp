...
function loadStore(): void {
  try {
    ...
    for (const token of store.accessTokens || [])
      if (Date.now() < token.expires_at) accessTokens.set(token.token, token);
...
function saveStore(): void {
  ...
  const store = {
    ...
    accessTokens: [...accessTokens.values()].filter(
...
function publicBaseUrl(req: IncomingMessage): string {
  const proto = String(req.headers["x-forwarded-proto"] || "https")
    ...
    .trim();
  const host =
    req.headers["x-forwarded-host"] || req.headers.host || "127.0.0.1:3939";
  return `${proto}://${host}`;
}
...
export function handleOAuthUserinfo(
  req: IncomingMessage,
  res: ServerResponse,
): void {
  ...
  const token = auth.startsWith("Bearer ")
    ...
    : "";
  if (!token || !isIssuedOAuthAccessToken(token))
    return json(
      res,
      401,
      { error: "invalid_token" },
...
export function handleOAuthJwks(
...
export async function handleOAuthRegistration(
  req: IncomingMessage,
  res: ServerResponse,
): Promise<void> {
  if (req.method !== "POST")
...
export function handleOAuthAuthorize(
  ...
  if (!client)
    return html(
      res,
      400,
      "OAuth client not found. Recreate or reconnect the MCP client so dynamic client registration can run.",
    );
  ...
  if ((url.searchParams.get("response_type") || "") !== "code")
    return redirectError(res, redirectUri, "unsupported_response_type", state);
...
function parseBasicAuth(req: IncomingMessage): {
  ...
  const [clientId, clientSecret] = Buffer.from(auth.slice(6), "base64")
    .toString()
    .split(":");
  return {
...
export async function handleOAuthToken(
  ...
  if (grantType === "authorization_code") {
    ...
    if (!code || code.client_id !== clientId || Date.now() > code.expires_at)
      return json(res, 400, { error: "invalid_grant" });
    if (
      params.get("redirect_uri") &&
      params.get("redirect_uri") !== code.redirect_uri
    )
      return json(res, 400, { error: "invalid_grant" });
    if (!verifyPkce(code, params.get("code_verifier") || ""))
      return json(res, 400, {
        error: "invalid_grant",
