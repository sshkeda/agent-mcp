...
import { z } from "zod";
import { deleteExpiredSessions, deleteSession, getOrCreateSession, sessionCount } from "./session.js";
...
const SESSION_IDLE_TIMEOUT_MS = parseInt(process.env.AGENT_MCP_SESSION_IDLE_TIMEOUT_MS ?? String(6 * 60 * 60 * 1000), 10);
...
function normalizeProfile(raw: string): AgentMcpProfile {
  ...
  if (value === "readonly" || value === "read-only" || value === "read") return "readonly";
  return "run-only";
}
...
function truncateForLog(value: unknown, max = 4000): unknown {
  ...
  return text.length > max ? `${text.slice(0, max)}...[truncated ${text.length - max} chars]` : value;
}

function logToolEvent(event: string, data: Record<string, unknown>): void {
  try {
    mkdirSync(LOG_DIR, { recursive: true });
    appendFileSync(TOOL_CALL_LOG, `${JSON.stringify({ ts: new Date().toISOString(), event, ...data })}\n`);
  } catch {
    // Logging must never break MCP tool execution.
  }
}

function addNonEnumerableToolAliases(server: McpServer, canonicalName: string, aliases: string[]): void {
...
function registerReadTool(server: McpServer): void {
  server.tool(
    ...
    {
      path: z.string().describe("File path. Use an absolute path for repo/project files outside the sandbox; relative paths resolve inside the session sandbox."),
      offset: z.number().optional().describe("0-based line number to start from; use with limit for large files"),
      limit: z.number().optional().describe("Maximum number of lines to return; use for large files"),
    },
    { title: "Read File", readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async (args) => {
      const session = getOrCreateSession(TOOL_SESSION_KEY);
      logToolEvent("tool_call_start", { tool: "read", session: session.id, cwd: session.cwd, args: truncateForLog(args) });
      try {
        const result = executeRead(session, args);
        logToolEvent("tool_call_end", { tool: "read", session: session.id, cwd: session.cwd, result: truncateForLog(result) });
        return { content: [{ type: "text", text: result }] };
      } catch (err: any) {
        logToolEvent("tool_call_error", { tool: "read", session: session.id, cwd: session.cwd, error: err.message });
...
function registerLowLevelTools(server: McpServer): void {
  server.tool(
    ...
    {
      command: z.string().describe("Shell command to execute. For repo/project commands, start with `cd /absolute/repo && ...` because the default cwd is the session sandbox."),
      timeout_seconds: z.number().optional().describe("Timeout in seconds (default: 120)"),
    },
    { title: "Execute Bash", readOnlyHint: false, destructiveHint: true, openWorldHint: true },
    async (args) => {
      const session = getOrCreateSession(TOOL_SESSION_KEY);
      logToolEvent("tool_call_start", { tool: "bash", session: session.id, cwd: session.cwd, args: truncateForLog(args) });
      const result = await executeBash(session, args);
      logToolEvent("tool_call_end", { tool: "bash", session: session.id, cwd: session.cwd, result: truncateForLog(result) });
      return { content: [{ type: "text", text: result }] };
    },
  );
  ...
  server.tool(
    ...
    { title: "Write File", readOnlyHint: false, destructiveHint: true, openWorldHint: false },
    async (args) => {
      const session = getOrCreateSession(TOOL_SESSION_KEY);
      logToolEvent("tool_call_start", { tool: "write", session: session.id, cwd: session.cwd, args: truncateForLog(args) });
      try {
        const result = executeWrite(session, args);
        logToolEvent("tool_call_end", { tool: "write", session: session.id, cwd: session.cwd, result: truncateForLog(result) });
        return { content: [{ type: "text", text: result }] };
      } catch (err: any) {
        logToolEvent("tool_call_error", { tool: "write", session: session.id, cwd: session.cwd, error: err.message });
        return { content: [{ type: "text", text: `[error: ${err.message}]` }], isError: true };
      }
    },
  );
  ...
  server.tool(
    ...
    "Apply atomic exact string replacements to a file. Each edit's oldText must appear exactly once. Use absolute paths for repo/project files outside the sandbox; relative paths resolve inside the session sandbox. Prefer read first to inspect context before editing. Editing can be destructive, so use it only when the user has authorized changes.",
    {
      ...
      edits: z.array(z.object({
        oldText: z.string().describe("Exact text to find; include enough surrounding context so it matches exactly once"),
        newText: z.string().describe("Replacement text"),
      })).describe("Array of {oldText, newText} replacements"),
    },
    { title: "Edit File", readOnlyHint: false, destructiveHint: true, openWorldHint: false },
    async (args) => {
      const session = getOrCreateSession(TOOL_SESSION_KEY);
      logToolEvent("tool_call_start", { tool: "edit", session: session.id, cwd: session.cwd, args: truncateForLog(args) });
      try {
        const result = executeEdit(session, args);
        logToolEvent("tool_call_end", { tool: "edit", session: session.id, cwd: session.cwd, result: truncateForLog(result) });
        return { content: [{ type: "text", text: result }] };
      } catch (err: any) {
        logToolEvent("tool_call_error", { tool: "edit", session: session.id, cwd: session.cwd, error: err.message });
        return { content: [{ type: "text", text: `[error: ${err.message}]` }], isError: true };
      }
    },
  );
...
function createMcpServer(): McpServer {
  ...
  server.tool(
    ...
    {
      code: z.string().describe("Async JavaScript function body. Available globals: repo, npm, shell, ui. repo.files(pattern) returns string paths. repo.read() accepts a string path or an object with a path/absolutePath field. Use repo.write(path, content) and repo.edit(path, { oldText, newText }) for file changes. Use an explicit return statement; a bare final expression returns null. Example: const scripts = await npm.findScripts('mcp debug'); const matches = await repo.search('buildNativeMcpPrompt'); return { script: scripts[0]?.name, file: matches[0]?.path ?? null };"),
      timeout_seconds: z.number().optional().describe("Execution timeout in seconds (default 30, max 120)"),
    },
    { title: "Run Agent SDK Code", readOnlyHint: false, destructiveHint: true, openWorldHint: true },
    async (args, extra) => {
      const session = getOrCreateSession(TOOL_SESSION_KEY);
      logToolEvent("tool_call_start", { tool: "run", session: session.id, cwd: currentCwdPin()?.cwd || session.cwd, pin: currentCwdPin(), args: truncateForLog(args) });
      try {
        const result = executeRun(session, args);
        logToolEvent("tool_call_end", { tool: "run", session: session.id, cwd: session.cwd, result: truncateForLog(result) });
        return { content: [{ type: "text", text: result }] };
      } catch (err: any) {
        logToolEvent("tool_call_error", { tool: "run", session: session.id, cwd: session.cwd, error: err.message });
        return { content: [{ type: "text", text: `[error: ${err.message}]` }], isError: true };
      }
    },
  );
...
function writeCorsHeaders(res: ServerResponse): void {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Authorization, X-Agent-MCP-Token, X-My-MCP-Token, X-PI-MCP-Token, Content-Type, MCP-Session-Id, mcp-session-id");
...
function tokenFromRequest(req: IncomingMessage, url = requestUrl(req)): string {
  ...
  for (const name of ["x-agent-mcp-token", "x-my-mcp-token", "x-pi-mcp-token"]) {
...
async function handleAdminPin(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (!isLocalAdminRequest(req)) return jsonResponse(res, 403, { ok: false, error: "admin endpoint is local-only" });
  let body: any;
  try { body = await parseJsonBody(req); } catch { return jsonResponse(res, 400, { ok: false, error: "request body must be JSON" }); }
  try {
    const state = pinCwd({ cwd: String(body.cwd || ""), epoch: Number(body.epoch), ttlMs: Number(body.ttlMs || 5 * 60_000) });
    logToolEvent("cwd_pin", { cwd: state.cwd, epoch: state.epoch, expiresAt: state.expiresAt });
...
async function handleAdminUnpin(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (!isLocalAdminRequest(req)) return jsonResponse(res, 403, { ok: false, error: "admin endpoint is local-only" });
  let body: any;
  try { body = await parseJsonBody(req); } catch { return jsonResponse(res, 400, { ok: false, error: "request body must be JSON" }); }
...
async function handleAdminRefreshTools(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (!isLocalAdminRequest(req)) return jsonResponse(res, 403, { ok: false, error: "admin endpoint is local-only" });
  const results = await Promise.allSettled(Object.values(mcpServers).map((server) => server.sendToolListChanged()));
  ...
  return jsonResponse(res, 200, { ok: true, notified, failed, activeServers: Object.keys(mcpServers).length, tools: toolNamesForDebug() });
}
...
const httpServer = createServer(async (req, res) => {
  ...
  if ((url.pathname === "/admin/debug" || url.pathname === "/admin/sessions" || url.pathname === "/admin/tools") && req.method === "GET") {
...
const cleanupTimer = setInterval(() => {
  ...
  for (const sessionId of expiredSessionIds) {
    void closeTransport(sessionId).catch((error) => {
      console.error(`Failed to close expired MCP transport ${sessionId}:`, error);
    });
  }
