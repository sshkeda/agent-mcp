...
export function pinCwd({
  ...
  if (!Number.isFinite(epoch) || epoch <= 0)
    throw new Error(`epoch must be a positive number: ${epoch}`);
  const current = currentCwdPin();
  if (current && current.epoch >= epoch)
    throw new Error(`stale epoch ${epoch}; current epoch is ${current.epoch}`);
  pinState = {
    cwd: resolve(cwd),
    epoch,
    expiresAt: Date.now() + Math.max(1_000, Math.min(ttlMs, 30 * 60_000)),
  };
  return pinState;
}

export function unpinCwd(epoch?: number): {
  cleared: boolean;
  alreadyCleared: boolean;
} {
  const current = currentCwdPin();
  if (!current) return { cleared: false, alreadyCleared: true };
  if (epoch !== undefined && Number.isFinite(epoch) && current.epoch !== epoch)
...
export function requirePinnedCwd(): CwdPinState {
  ...
  if (!current)
    throw new Error(
      "native_mcp_unbound: no cwd is pinned for this native MCP session",
    );
  return current;
}
